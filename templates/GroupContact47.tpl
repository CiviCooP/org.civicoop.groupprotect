    ...
    {strip}
      <table id="current_group" class="display">
        ...
        {foreach from=$groupIn item=row}
          ...
          {crmAPI var='groupProtected' entity='Group' action='getprotect' group_id=$row.group_id}
          {foreach from=$groupProtected.values item=apiGroup}
            {if $apiGroup.protected AND !$userHasProtectGroup}
        ...
        {foreach from=$groupOut item=row}
          ...
          <tr id="group_contact-{$row.id}" class="crm-entity {cycle values="odd-row,even-row"}">
            ...
            {if $permission EQ 'edit' AND $actionItemsAllowed}
              ...
              <a class="action-item crm-hover-button" href="#Deleted" title="{ts escape='htmlattribute' 1=$displayName 2=$row.title}Delete %1 from %2? (this group will no longer be listed under Past Groups).{/ts}">
                {ts}Delete{/ts}</a>{/if}
            </td>
          </tr>
        {/foreach}
...
<script type="text/javascript">
  CRM.$(function($) {
    ...
    // function to show only protected groups if relevant
    $('.s2id_group_id').click(function() {
      console.log('Ik ben er !');
      $('.select2-match').each(function() {
        console.log('ik heb een select2-match');
      })
    });
  });
